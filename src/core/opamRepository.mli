...
val init: repository -> unit OpamParallel.job

(** Update {i $opam/repo/$repo}. *)
val update: repository -> unit OpamParallel.job

(** Error and exit on incompatible version *)
val check_version: repository -> unit OpamParallel.job
...
val pull_url: repository_kind ->
  ...
  generic_file download OpamParallel.job
...
val pull_archive: repository -> package -> filename download OpamParallel.job
...
(** [make_archive ?gener_digest repo prefix package] builds the
    ...
    [gener_digest] is set. *)
val make_archive: ?gener_digest:bool -> repository -> string option -> package -> unit OpamParallel.job
